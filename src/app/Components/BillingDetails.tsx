...
const BillingDetails = () => {
    const form = useForm({
        ...
        validate: {
            Fname: (value) => (value.length < 2 ? 'First Name must have at least 3 letters' : null),
            Lname: (value) => (value.length < 2 ? 'Last Name must have at least 3 letters' : null),
            Phone: (value) => (/^\d{10}$/.test(value) ? null : 'Invalid email'),
            Email: (value) => (/^\S+@\S+$/.test(value) ? null : 'Invalid email'),
            Address1: (value) => (value.length < 5 ? 'Correct the Address 01' : null),
            Address2: (value) => (value.length < 5 ? 'Correct the Address 02' : null),
            notes: (value) => (value.length < 5 ? 'Add Notes' : null),
            TownCity: (value) => (value.length < 5 ? 'Add Town / City' : null),
          },
        // validate: (values) => ({
            ...
            <div className="px-3 lg:w-2/3">
                ...
                <h3 className='mb-[30px] pb-[10px] text-[18px] font-medium'>Billing Details</h3>
                    <div className="flex flex-col">
                        <div className="flex flex-col md:flex-row w-full px-[15px] md:gap-[30px]" >
